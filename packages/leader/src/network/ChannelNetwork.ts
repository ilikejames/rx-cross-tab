import { Observable, filter, fromEvent, map, share, take, tap } from 'rxjs'
...
export class ChannelNetwork<T extends string> {
    ...
    constructor(private channelName: string, public readonly instanceId: string, private logger?: Logger) {
    ...
    public send(message: Pick<Message<T>, 'topic' | 'payload'> & Partial<Message<T>>, type?: string) {
        const request: Message<T> = {
            from: this.instanceId,
            ...message,
