...
import { VoteBy, VoteFor, ElectionResults } from './types';
...
export class ElectionSvc {
    ...
    constructor(
        ...
        this.electionCompleteMs =
            this.options.electionTimeoutMin + this.options.electionTimeoutRange;
        this.channel = new ChannelNetwork(
            this.options.electionChannelName,
            this.instanceId,
            this.logger,
        );
    ...
    private handleVoteRequested(message: VoteForMe) {
        ...
        if (!this.electionTimerId) {
            ...
            this.logger?.debug(
                loggerName,
                `New election requested, completing in ${remainingOfElection}ms`,
            );
            this.hasVoted = false;
    ...
    public start() {
        ...
        this.logger?.debug(
            loggerName,
            `Election ends at ${endOfElectionTime}`,
            this.electionCompleteMs,
        );
